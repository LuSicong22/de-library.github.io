...
let account = null;
let libraryContract;
let libraryContractAddress = "TM7kFRhfW6AmTKDKknAp8qbRRMzvWiciEb"; // Paste Contract address here
...
export async function borrowBook(
  ...
  const result = await bookRentContract
    .borrowBook(spaceId, checkInDate, checkOutDate)
    .send({
      feeLimit: 100_000_000,
  ...
  for (let i = 0; i < bookId; i++) {
    ...
    if (book.name != "") {
      ...
      books.push({
        ...
        price: window.tronWeb.fromSun(book.price),
      });
    }
  }
  return books;
